...
class Ebook_manager extends CI_Controller
{
    ...
    public function add_flipbooks()
    {
        $page_data['page_title'] = 'Add flipbooks';
        $page_data['page_name'] = 'add_flipbooks';
        $this->load->view('backend/index', $page_data);
    }

    public function edit_flipbook($ebook_id = " ")
    {
        #$page_data['ebook'] = $this->ebook_model->get_ebooks_list($ebook_id)->row_array();
        $page_data['ebook'] = $this->db->where('ebook_id', $ebook_id)->order_by('ebook_id', 'desc')->get('ebook')->row_array();
        $page_data['page_title'] = get_phrase('edit_ebook');
        $page_data['page_name'] = 'flipbook_edit';
        $this->load->view('backend/index', $page_data);
    }

    public function page_data($ebook_id, $action, $param1 = null)
    {
        //$order   = $this->input->post('order') ;
        $ebook = $this->db->where('ebook_id', $ebook_id)->get('ebook')->row_array();
        ...
        if ($action == 'add') {
            ...
            if ( isset($flipbookContent['pages']) &&  count($flipbookContent['pages']) > 0 ) {
                $pageOrders = array_keys($flipbookContent['pages']);
                $lastOrder = end($pageOrders);
            }else{
                $lastOrder = 0;
            }
            $newOrder = $lastOrder + 1;
            $flipbookContent['pages'][$newOrder] = htmlspecialchars($content);

            $this->db->where('ebook_id', $ebook_id);
            $this->db->update('ebook', ['flipbook_content' => json_encode($flipbookContent, JSON_UNESCAPED_UNICODE)]);
        } elseif ($action == 'edit') {
            
            $content = $this->input->post('content',false);
            $flipbookContent['pages'][$param1] = htmlspecialchars($content);

            $this->db->where('ebook_id', $ebook_id);
            $this->db->update('ebook', ['flipbook_content' => json_encode($flipbookContent, JSON_UNESCAPED_UNICODE)]);
        } else {
            if (isset($flipbookContent['pages'][$param1])) {
                // Delete the key
                unset($flipbookContent['pages'][$param1]);

                if ( count($flipbookContent['pages']) > 0 ) {
                    $x = 1;
                    foreach ($flipbookContent['pages'] as $orders => $content) {
                        $flipbookContentNew['pages'][$x] = $content;
                        $x++;
                    }
                    $flipbookContent = json_encode($flipbookContentNew, JSON_UNESCAPED_UNICODE) ;
                }else{
                    $flipbookContent = null;
                }
                // Display the updated content
                $this->db->where('ebook_id', $ebook_id);
                $this->db->update('ebook', ['flipbook_content' => $flipbookContent]);
            }
        }

        return redirect(site_url("addons/ebook_manager/edit_flipbook/$ebook_id"), 'refresh');
    }
}
