...
class Ebook extends CI_Controller
{
    ...
    //code mouaad 
    function download_ebook($ebook_id)
    {
        $filesDirectory = 'uploads/ebook/file/ebook_full/';
        $ebook = $this->ebook_model->get_ebook_by_id($ebook_id)->row_array();
        if (($ebook['is_download'] == 1) && (($this->db->get_where('ebook_payment', array('user_id' => $this->session->userdata('user_id'), 'ebook_id' => $ebook_id))->num_rows() > 0) || $ebook['is_free'] || ($this->session->userdata('user_id') == $ebook['user_id']) || (strtolower($this->session->userdata('role') ?? '') == "admin"))) {
            $filePath = $filesDirectory . $ebook['file'];
