...
class User_model extends CI_Model
{
    ...
    { // Admin does this editing
        ...
        if ($validity) {
            $data['first_name'] = $this->str_without_accents(html_escape($this->input->post('first_name')));
            $data['last_name'] = $this->str_without_accents(html_escape($this->input->post('last_name')));
    ...
    function set_login_userdata($user_id = "")
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                } elseif (($this->db->where('user_id', $user_id)->get("ebook_payment")->num_rows()) > 0) {
                    redirect(site_url('home/my_ebooks'), 'refresh');
                }
