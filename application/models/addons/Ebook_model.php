...
class Ebook_model extends CI_Model
{
    ...
    public function get_instructor_wise_ebook_ratings($instructor_id = "", $ratable_type = "", $is_sum = false)
    {
        ...
        if ($is_sum) {
            ...
            $this->db->where_in('ratable_id', $ebook_ids);
            $this->db->select_sum('rating');
    ...
    // code mouaad ///////////////////////////////////////////////
    public function get_status_wise_ebooks_for_instructor($status, $allUser = false)
    {
        if (!$allUser) {
            $user_id = $this->session->userdata('user_id');
            if ($status == 1) {
                $this->db->where(['is_active' => 1, 'user_id' => $user_id]);
            } else {
                $this->db->where(['is_active' => 0, 'user_id' => $user_id]);
            }
        } else {
            if ($status == 1) {
                $this->db->where(['is_active' => 1]);
    ...
    {
        $query = $this->db
            ->select("category_id, count(*) AS ebook_number", false)
            ->from("ebook")
            ->group_by('category_id')
            ->order_by("ebook_number", "DESC")
            ->where('is_active', 1)
            ->limit($limit)
            ->get();
